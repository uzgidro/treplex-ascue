...
$this->title = '"Ўрта Чирчиқ ГЭСлар каскади" Филиалида электр энергия ишлаб чиқарилиши';
    ...
    <div class="col-5 col-xl-5">
        <div class="row g-3 list">
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([24], 'ges6', 6) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([25], 'ges27', 27) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([26], 'ges28', 28) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([27], 'ispaysoy', "ispaysoy") ?>
            </div>
        </div>
    </div>
    <div class="col-7 col-xl-7">
        <?= $dashboard->AllResultGes(1) ?>
        ...
        <script>
            ...
            ?>
            const data6 = <?= $dashboard->YesterdayHourGes(24) ?>;
            const data27 = <?= $dashboard->YesterdayHourGes(25) ?>;
            const data28 = <?= $dashboard->YesterdayHourGes(26) ?>;
            const dataIspay = <?= $dashboard->YesterdayHourGes(27) ?>;
...
<script>
    ...
    function getServer() {
        ...
        fetch('/urta-chirchiq/server', { signal: controller.signal })
            .then(response => {
                if (!response.ok) {
            ...
            .then(text => {
                ...
                data.forEach(item => {
                    if(item.id === 1) {
                        ...
                        const value = document.getElementById("active27");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 15) {
                        const value = document.getElementById("true27");
                        value.textContent = item.value;
                    } else if(item.id === 16) {
                        const value = document.getElementById("false27");
                        value.textContent = item.value;
                    } else if(item.id === 17) {
                        const value = document.getElementById("active28");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 18) {
                        const value = document.getElementById("true28");
                        value.textContent = item.value;
                    } else if(item.id === 19) {
                        const value = document.getElementById("false28");
                        value.textContent = item.value;
                    } else if(item.id === 20) {
                        const value = document.getElementById("error28");
                        value.textContent = item.value;
                    }
                })
