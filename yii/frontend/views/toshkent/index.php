...
$this->title = '"Тошкент ГЭСлар каскади" Филиалида электр энергия ишлаб чиқарилиши';
...
<div class="row g-3 list mb-3">
    <div class="col-5 col-xl-5">
        <div class="row g-3 list">
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([37], 'ges1', 1) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([38], 'ges21', 21) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([39], 'ges4', 4) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([40], 'ges9', 9) ?>
            </div>
        </div>
    </div>
    <div class="col-7 col-xl-7">
        ...
        <div class="card mt-1" style="backdrop-filter: blur(5px);">
            <div class="card-body">
...
<script>
    ...
    ?>
    const dataDay1 = <?= $dashboard->YesterdayHourGes(37) ?>;
    const dataDay21 = <?= $dashboard->YesterdayHourGes(38) ?>;
    const dataDay4 = <?= $dashboard->YesterdayHourGes(39) ?>;
    const dataDay9 = <?= $dashboard->YesterdayHourGes(40) ?>;
    ...
    function getServer() {
        ...
        fetch('/toshkent/server', { signal: controller.signal })
            .then(response => {
                if (!response.ok) {
            ...
            .then(text => {
                ...
                data.forEach(item => {
                    if(item.id === 1) {
                        ...
                        const value = document.getElementById("active21");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 15) {
                        const value = document.getElementById("true21");
                        value.textContent = item.value;
                    } else if(item.id === 16) {
                        const value = document.getElementById("false21");
                        value.textContent = item.value;
                    } else if(item.id === 17) {
                        const value = document.getElementById("error21");
                        value.textContent = item.value;
                    }
