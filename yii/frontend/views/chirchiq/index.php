...
$this->title = '"Чирчиқ ГЭСлар каскади" Филиалида электр энергия ишлаб чиқарилиши';
...
<div class="row g-3 list mb-2">
    <div class="col-6 col-xl-6">
        <div class="row g-3 list">
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([28], 'ges8', 8) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([29], 'ges7', 7) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([31], 'ges10', 10) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([30], 'ges46', 46) ?>
            </div>
        </div>
    </div>
    <div class="col-6 col-xl-6">
        <?= $dashboard->AllResultGes(2) ?>
    </div>
</div>

<div class="card mb-2" style="backdrop-filter: blur(5px);">
    <div class="card-body">
...
<script>
    ...
    ?>
    const data8 = <?= $dashboard->YesterdayHourGes(28) ?>;
    const data7 = <?= $dashboard->YesterdayHourGes(29) ?>;
    const data10 = <?= $dashboard->YesterdayHourGes(31) ?>;
    const dataKamolot = <?= $dashboard->YesterdayHourGes(30) ?>;
...
<script>
    ...
    function getServer() {
        ...
        fetch('/chirchiq/server', { signal: controller.signal })
            .then(response => {
                if (!response.ok) {
            ...
            .then(text => {
                ...
                data.forEach(item => {
                    if(item.id === 1) {
                        ...
                        const value = document.getElementById("active46");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 23) {
                        const value = document.getElementById("true46");
                        value.textContent = item.value;
                    } else if(item.id === 24) {
                        const value = document.getElementById("false46");
                        value.textContent = item.value;
                    } else if(item.id === 25) {
                        const value = document.getElementById("error46");
                        value.textContent = item.value;
                    }
                })
            })
