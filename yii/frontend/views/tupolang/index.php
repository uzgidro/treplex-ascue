...
$this->title = '"Тўполанг ГЭС" Филиалида электр энергия ишлаб чиқарилиши';
...
<div class="row g-2 list mb-2">
    <div class="col-5 col-xl-5">
        <div class="row g-2 list">
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([71], 'ges34', 34) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([72], 'ges43', 43) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([73], 'ges44', 44) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([74], 'ges1', 1) ?>
            </div>
        </div>
    </div>
    <div class="col-7 col-xl-7">
        <?= $dashboard->AllResultGes(15) ?>
        <div class="card mt-2" style="backdrop-filter: blur(5px);">
            <div class="card-body">
                <div class="row g-1 g-sm-3 lh-1">
                    <canvas id="myLineChart" width="1500" height="250"></canvas>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    ?>
    const data34 = <?= $dashboard->YesterdayHourGes(71) ?>;
    const data43 = <?= $dashboard->YesterdayHourGes(72) ?>;
    const data44 = <?= $dashboard->YesterdayHourGes(73) ?>;
    const dataIsfara = <?= $dashboard->YesterdayHourGes(74) ?>;
...
<script>
    ...
    function getServer() {
        ...
        fetch('/tupolang/server', { signal: controller.signal })
            .then(response => {
                if (!response.ok) {
            ...
            .then(text => {
                ...
                data.forEach(item => {
                    if(item.id === 1) {
                        ...
                        const value = document.getElementById("active34");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 12) {
                        const value = document.getElementById("true34");
                        value.textContent = item.value;
                    } else if(item.id === 13) {
                        const value = document.getElementById("false34");
                        value.textContent = item.value;
                    } else if(item.id === 14) {
                        const value = document.getElementById("active43");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 15) {
                        const value = document.getElementById("true43");
                        value.textContent = item.value;
                    } else if(item.id === 16) {
                        const value = document.getElementById("false43");
                        value.textContent = item.value;
                    } else if(item.id === 17) {
                        const value = document.getElementById("active44");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 18) {
                        const value = document.getElementById("true44");
                        value.textContent = item.value;
                    } else if(item.id === 19) {
                        const value = document.getElementById("false44");
                        value.textContent = item.value;
                    }
                })
