...
$this->title = '"Қамчиқ ГЭС" Филиалида электр энергия ишлаб чиқарилиши';
    ...
    <div class="col-4 col-xl-4">
        <div class="row g-3 list">
            <div class="col-12 col-xl-12">
                <?= $dashboard->Ges([78], 'ges42', 42) ?>
            </div>
        </div>
    </div>
    <div class="col-8 col-xl-8">
        <?= $dashboard->AllResultGes(23) ?>
    </div>
</div>
...
<script>
    ...
    ?>
    const dataDay = <?= $dashboard->YesterdayHourGes(78) ?>;
...
<script>
    ...
    function getServer() {
        ...
        fetch('/qamchiq/server', { signal: controller.signal })
            .then(response => {
                if (!response.ok) {
            ...
            .then(text => {
                ...
                data.forEach(item => {
                    if(item.id === 1) {
                        ...
                        const value = document.getElementById("active42");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 11) {
                        const value = document.getElementById("true42");
                        value.textContent = item.value;
                    } else if(item.id === 12) {
                        const value = document.getElementById("false42");
                        value.textContent = item.value;
                    } else if(item.id === 13) {
                        const value = document.getElementById("error42");
                        value.textContent = item.value;
                    }
                })
