...
$this->title = '"Хисорак ГЭС" Филиалида электр энергия ишлаб чиқарилиши';
...
<div class="row g-3 list mb-3">
    <div class="col-6 col-xl-6">
        <div class="row g-3 list">
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([77], 'ges37', 37) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([88], '#', "samak") ?>
            </div>
...
<script>
    ...
    ?>
    const data16 = <?= $dashboard->YesterdayHourGes(77) ?>;
...
<script>
    ...
    function getServer() {
        ...
        fetch('/xisorak/server', { signal: controller.signal })
            .then(response => {
                if (!response.ok) {
            ...
            .then(text => {
                ...
                data.forEach(item => {
                    if(item.id === 1) {
                        ...
                        const value = document.getElementById("active37");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 11) {
                        const value = document.getElementById("true37");
                        value.textContent = item.value;
                    } else if(item.id === 12) {
                        const value = document.getElementById("false37");
                        value.textContent = item.value;
                    } else if(item.id === 13) {
                        const value = document.getElementById("error37");
                        value.textContent = item.value;
                    }
