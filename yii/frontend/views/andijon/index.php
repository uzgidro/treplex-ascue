...
$this->title = '"Андижон ГЭС" Филиалида электр энергия ишлаб чиқарилиши';
...
<div class="row g-3 list mb-3">
    <div class="col-6 col-xl-6">
        <div class="row g-3 list">
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([11], 'ges29', 29) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([59], 'ges36', 36) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([60], "kudash", "kudash") ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([62], 'xonobod', 'xonobod') ?>
            </div>
        </div>
    </div>
    <div class="col-6 col-xl-6">
        <?= $dashboard->AllResultGes(10) ?>
    </div>
    <div class="col-12 col-xl-12" style="backdrop-filter: blur(15px);">
...
<script>
    ...
    ?>
    const data29 = <?= $dashboard->YesterdayHourGes(11) ?>;
    const data36 = <?= $dashboard->YesterdayHourGes(59) ?>;
    const dataKudash = <?= $dashboard->YesterdayHourGes(60) ?>;
...
<script>
    ...
    function getServer() {
        ...
        fetch('/andijon/server', { signal: controller.signal })
            .then(response => {
                if (!response.ok) {
            ...
            .then(text => {
                ...
                data.forEach(item => {
                    if(item.id === 1) {
                        ...
                        const value = document.getElementById("active29");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 11) {
                        const value = document.getElementById("true29");
                        value.textContent = item.value;
                    } else if(item.id === 12) {
                        const value = document.getElementById("false29");
                        value.textContent = item.value;
                    } else if(item.id === 13) {
                        const value = document.getElementById("error29");
                        value.textContent = item.value;
                    }

                    else if(item.id === 14) {
                        const value = document.getElementById("active36");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 15) {
                        const value = document.getElementById("true36");
                        value.textContent = item.value;
                    } else if(item.id === 16) {
                        const value = document.getElementById("false36");
                        value.textContent = item.value;
                    } else if(item.id === 17) {
                        const value = document.getElementById("error36");
                        value.textContent = item.value;
                    }

                    else if(item.id === 18) {
                        const value = document.getElementById("activekudash");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 19) {
                        const value = document.getElementById("truekudash");
                        value.textContent = item.value;
                    } else if(item.id === 20) {
                        const value = document.getElementById("falsekudash");
                        value.textContent = item.value;
                    } else if(item.id === 21) {
                        const value = document.getElementById("errorkudash");
                        value.textContent = item.value;
                    }
                })
    ...
    var intervalTime = 300000; // Set the interval time in milliseconds (e.g., 1000ms = 1 second)
