...
$this->title = '"Фарход ГЭС" Филиалида электр энергия ишлаб чиқарилиши';
...
<div class="row g-3 list mb-2">
    <div class="col-6 col-xl-6">
        <div class="row g-3 list">
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([7], 'ges16', 16) ?>
            </div>
            <div class="col-6 col-xl-6">
                <?= $dashboard->Ges([48], 'zomin', 'zomin') ?>
            </div>
            <div class="col-12 col-xl-12">
                <div class="card" style="backdrop-filter: blur(5px);">
                    <div class="card-body">
                        <div class="row g-1 g-sm-3 lh-1">
                            <canvas id="myLineChart" width="1500" height="400"></canvas>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    ?>
    const data16 = <?= $dashboard->YesterdayHourGes(7) ?>;
    const dataZomin = <?= $dashboard->YesterdayHourGes(48) ?>;
            ...
            .then(text => {
                ...
                data.forEach(item => {
                    if(item.id === 1) {
                        ...
                        const value = document.getElementById("active16");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 11) {
                        const value = document.getElementById("true16");
                        value.textContent = item.value;
                    } else if(item.id === 12) {
                        const value = document.getElementById("false16");
                        value.textContent = item.value;
                    } else if(item.id === 13) {
                        const value = document.getElementById("error16");
                        value.textContent = item.value;
                    }

                    else if(item.id === 14) {
                        const value = document.getElementById("active17");
                        value.textContent = formatNumber(item.value, 2, ',', ' ') + " МВт";
                    } else if(item.id === 15) {
                        const value = document.getElementById("true17");
                        value.textContent = item.value;
                    } else if(item.id === 16) {
                        const value = document.getElementById("false17");
                        value.textContent = item.value;
                    } else if(item.id === 17) {
                        const value = document.getElementById("error17");
                        value.textContent = item.value;
                    }
                })
